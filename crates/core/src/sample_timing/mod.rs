...
impl SampleTiming {
	...
	pub fn sample_clock_with_frequency(&self, frequency: f32) -> f32 {
		let sample_clock_length =
			(1.0 / frequency * self.sample_rate * frequency.ceil()) as usize;
		((self.clock + sample_clock_length) % sample_clock_length) as f32
			/ self.sample_rate
	}

	pub fn sample_clock_with_length(&self, length: f32) -> f32 {
		let sample_clock_length =
			(length * self.sample_rate * (1.0 / length).ceil()) as usize;
		((self.clock + sample_clock_length) % sample_clock_length) as f32
			/ self.sample_rate
	}
