...
impl Effect for Lag {
	fn process(
		...
		if let Some((ref mut index, ref mut buffer)) =
			self.active_indexed_buffer
		{
			...
			if rng.gen::<f32>() < self.start_chance {
				let active_buffer_length = self.buffer_length
					+ rng.gen_range(-1.0..1.0) * self.buffer_length_rand_bias;
				let active_buffer_size = sample_timing
					.duration_to_sample_count(active_buffer_length);
		...
		let buffer_size = sample_timing.duration_to_sample_count(
			self.buffer_length + self.buffer_length_rand_bias,
		);
