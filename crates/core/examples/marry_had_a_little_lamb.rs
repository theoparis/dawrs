			...
			if self.melody_index < self.melody.len() {
				...
				if self.current_note_quarter_count == 0 {
					...
					self.synth.base_generator.frequency =
						midi_id_to_frequency(note); //set frequency of synth to right note
					self.synth.play(
						quarter_duration * note_length as f32
							- 0.5 * quarter_duration,
					);
...
fn main() {
	...
	let patch = MelodyPatch {
		...
		melody: vec![
			76, 74, 72, 74, 76, 76, 76, 74, 74, 74, 76, 79, 79, 76, 74, 72, 74,
			76, 76, 76, 76, 74, 74, 76, 74, 72,
		],
		note_lengths: vec![
